...
### 快速安装（推荐）：
``` Bash
wget --no-check-certificate -O dnsmasq_sniproxy.sh https://raw.githubusercontent.com/kankankankankankan/dnsmasq_sniproxy_install/master/dnsmasq_sniproxy.sh && bash dnsmasq_sniproxy.sh -f
```

### 普通安装：
``` Bash
wget --no-check-certificate -O dnsmasq_sniproxy.sh https://raw.githubusercontent.com/kankankankankankan/dnsmasq_sniproxy_install/master/dnsmasq_sniproxy.sh && bash 
dnsmasq_sniproxy.sh -i
```

### 卸载方法：
``` Bash
wget --no-check-certificate -O dnsmasq_sniproxy.sh https://raw.githubusercontent.com/kankankankankankan/dnsmasq_sniproxy_install/master/dnsmasq_sniproxy.sh && bash dnsmasq_sniproxy.sh -u
```
...
- 重启
   ...
   ```
- 确认是否成功 `nslookup foxplus.com` 
- 若执行失败 CENTOS安装：`yum install -y bind-utils` DEBIAN安装：`apt-get -y install dnsutils`
   ```
   Name:   foxplus.com
   Address: vps-ip
